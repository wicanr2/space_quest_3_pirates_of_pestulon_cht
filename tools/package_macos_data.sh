...
# 注入宇宙傳奇 III 繁中資料(dist-cht/ 的 translation.tsv + qfg1_big5.fnt + qfg1_big5_hi.fnt
# + sq3_title.ovl)+ README,重新打包成可交付檔。在 CI runner 內跑(bash 內建即可,不需 docker/python)。
#
# SQ3 只有單一 SCI0 EGA 版本(不像 QFG1/LSL1 要分 vga/ega),故不帶 edition 參數。
#
...
[ -f "$ROOT/dist-cht/sq3_title.ovl" ] && cp "$ROOT/dist-cht/sq3_title.ovl" "$CHT_DIR/"   # 中文標題疊圖
echo ">>    staged $(ls "$CHT_DIR" | wc -l) 個中文資料檔 → $CHT_DIR"

README="$APP/Contents/Resources/README-cht.txt"
cat > "$README" <<'EOF'
宇宙傳奇 III：佩斯圖倫的海盜（Space Quest III: The Pirates of Pestulon）繁體中文化 — macOS 版

本包內容
--------
- patched ScummVM 執行檔（含 Big5 繪字、ZH_TWN 語言支援、hi-res 640x400 對白文字的引擎改動）
- cht-data/：中文資料（translation.tsv 對白/訊息、qfg1_big5.fnt 低解析字型、qfg1_big5_hi.fnt hi-res 字型、sq3_title.ovl 中文標題疊圖）
- 本說明檔

本包【不含】原遊戲資源，也不含 Roland MT-32 ROM（版權因素不隨包分發）。
請自備合法取得的宇宙傳奇 III（SCI/DOS 版）遊戲檔。
...
repo：https://github.com/wicanr2/space_quest_3_pirates_of_pestulon_cht.git
EOF
