...
#
# 前提：先從 CI artifact 下載 engine-only tar.gz（gh run download ... --name sq3-cht-macos）。
...
GAME_SRC="${2:-/home/anr2/scummvm/space_quest3/workplace/game}"
ROM_SRC="${3:-/home/anr2/cht/mt32}"
OUT="/home/anr2/scummvm/space_quest3/dist-all/macos"
WORK="$(mktemp -d)"; APP="$WORK/ScummVM.app"

tar xzf "$CI_TAR" -C "$WORK"
[ -d "$APP" ] || { echo "!! CI tar 內找不到 ScummVM.app" >&2; exit 1; }

# 1) 統一 game 夾：遊戲資源（SQ3 SCI0 為小寫 resource.*）+ cht 對白/字型/ovl
GAME="$APP/Contents/Resources/game"; mkdir -p "$GAME"
cp "$GAME_SRC"/resource.* "$GAME/"
cp "$GAME_SRC"/translation.tsv "$GAME_SRC"/qfg1_big5.fnt "$GAME_SRC"/qfg1_big5_hi.fnt "$GAME_SRC"/sq3_title.ovl "$GAME/"
# 2) MT-32 ROM（正名）
cp "$ROM_SRC"/MT32_CONTROL.1987-10-07.v1.07.ROM "$GAME/MT32_CONTROL.ROM"
cp "$ROM_SRC"/MT32_PCM.ROM "$GAME/MT32_PCM.ROM"
# 3) 啟動包裝：binary 改名 + wrapper 帶 SQ3 中文 + MT-32 參數
...
echo "完成！雙擊 ScummVM.app 開始《宇宙傳奇 III：佩斯圖倫的海盜》。"
read -n1 -p "按任意鍵關閉…"
FIX
chmod +x "$WORK/修復-macOS.command"
cat > "$APP/Contents/Resources/README-cht.txt" <<'RM'
宇宙傳奇 III：佩斯圖倫的海盜 — 繁體中文化（macOS 完整包，開箱即玩）
...
# 6) 打包
mkdir -p "$OUT"
( cd "$WORK" && tar czf "$OUT/SQ3-CHT-macos-universal-full.tar.gz" "ScummVM.app" "修復-macOS.command" )
echo "完成 → $OUT/SQ3-CHT-macos-universal-full.tar.gz ($(du -h "$OUT/SQ3-CHT-macos-universal-full.tar.gz" | cut -f1))"
rm -rf "$WORK"
